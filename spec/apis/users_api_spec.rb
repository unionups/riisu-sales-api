...
RSpec.describe 'Users API', type: :api do
  ...
  describe "AUTHENTICATED USER" do
    ...
    it "must be PATCH api/v1/user/:id -> users#update" do
      patch api_v1_user_path(user), {user: attributes_for(:user, first_name: 'Jack')}, { "HTTP_TOKEN" => user.auth_token }
  ...
  describe "AUTHENTICATED ADMIN" do
    ...
    it "must be PATCH api/v1/users/:id -> admin/users#update" do
      patch "/api/v1/users/#{user.id}", {user: attributes_for(:user, access_level: 1)}, { "HTTP_TOKEN" => admin.auth_token }
      expect(last_response.status).to eq 200
